...
INTERFACEVIEW=plex-odometry_iv.aadl
grep "version => \"2" plex-odometry_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view plex-odometry_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f dummy_mc_producer.zip && zip dummy_mc_producer dummy_mc_producer/* && cd $OLDPWD

cd "$SKELS" && rm -f dummy_pose_consumer.zip && zip dummy_pose_consumer dummy_pose_consumer/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	--subCPP dummy_mc_producer:"$SKELS"/dummy_mc_producer.zip \
	--subCPP dummy_pose_consumer:"$SKELS"/dummy_pose_consumer.zip \
